...
%/$(UPTODATE): %/Dockerfile
	for arch in $(ARCHS); do \
		$(SUDO) docker buildx build --platform linux/$$arch --load --build-arg=revision=$(GIT_REVISION) --build-arg=goproxyValue=$(GOPROXY_VALUE) -t $(IMAGE_PREFIX)$(shell basename $(@D)) -t $(IMAGE_PREFIX)$(shell basename $(@D)):$(IMAGE_TAG)-$$arch $(@D)/ ; \
	done
	@echo
